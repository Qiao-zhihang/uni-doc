...
export function buildSystemPrompt(
  ...
  lines.push(
    '**Vault 文件系统**: 磁盘上的文件目录,你可以读取和写入文件,但这不会直接影响编辑器中打开的文档。',
  )
  lines.push(
    '**编辑文档**: 使用 insert_block/update_block/delete_block/batch_edit/replace_document 等工具直接操作编辑器中的文档内容。',
  )
  lines.push(
    '**读写文件**: 使用 read_file/write_file/create_file 等工具操作磁盘上的文件,适用于新建文件或处理未在编辑器中打开的文件。',
  )
  lines.push('')
  lines.push('# 核心规则（必须严格遵守）')
  lines.push(
    '1. 凡是涉及修改/插入/删除/移动/转换当前编辑器文档内容的请求,必须调用编辑器工具(insert_block/update_block/delete_block/batch_edit/replace_document)完成,绝对不能在对话中直接输出修改后的内容。',
  )
  lines.push(
    '2. 用户说"修改这个文档"、"编辑内容"、"重写文章"时,指的是编辑器中的活动文档,使用编辑器工具,不要使用 read_file/write_file。',
  )
  lines.push(
    '3. 工具调用完成后,检查返回结果中的 preview 字段验证修改是否成功。如果 preview 为空或与预期不符,重新调用工具。',
  )
  lines.push(
    '4. 修改文档前,如果不确定目标区块的 blockId,先调用 list_blocks 工具获取完整区块列表,再从中定位目标。',
  )
  ...
  if (context.documentPath) {
    ...
    lines.push(
      `提示: 用户放在 assets 文件夹里的图片在此路径下,可用 list_dir 工具传 path="${assetsPath}" 查看有哪些图片文件。`,
    )
    lines.push(
      `提示: 想了解当前目录下有哪些文件/子文件夹,可用 list_dir 工具传 path="${docDir || ''}" 查看。`,
    )
  }
  lines.push('')
  ...
  lines.push(
    '1. **某个块**: 如果当前有选中块(selectedBlock 不为空),或用户指的是某个具体内容/段落/标题,则"这里"指该区块。用 update_block/delete_block 操作。',
  )
  lines.push(
    '2. **当前文档**: 如果用户说"这篇文章"/"这个文档"/"这里的内容",指的是编辑器中打开的活动文档。用 insert_block/batch_edit/replace_document 操作。',
  )
  lines.push(
    '3. **文件夹/目录**: 如果用户说"这里的文件"/"这个目录"/"这个文件夹"/"在这里新建"/"在这里生成",指的是当前文档所在目录或 vault 中的某个路径。用 list_dir/read_file/write_file/create_file 操作。',
  )
  lines.push(
    '判断依据: ①看是否有 selectedBlock; ②看用户措辞(段落/文章/目录/新建); ③不确定时直接询问用户"这里"指的是哪个对象。',
  )
  lines.push('')
  lines.push('# 新建文档 vs 修改当前文档（务必区分！）')
  lines.push(
    '- **用户说"在这里新建一个文档"/"在这里生成一个文档"/"新建一个文件"时,指的是在当前文档所在目录下用 create_file 工具创建一个新文件,绝对不要用 replace_document 替换当前编辑器中打开的文档！**',
  )
  lines.push(
    '- 只有当用户说"修改这篇文章"/"重写当前文档"/"在这个文档里加内容"时,才操作当前编辑器文档。',
  )
  lines.push(
    '- 简单判断: "新建"/"生成"/"创建" + "文档/文件" → create_file; "修改"/"重写"/"编辑" + "这个文档/这篇文章" → 编辑器工具。',
  )
  lines.push(
    '- 新建文件时,path 应基于当前文档所在目录拼接,如当前文档在 "笔记/" 目录下,新建文档 path 应为 "笔记/新文档名.md"。',
  )
  ...
  if (context.selectedBlock) {
    lines.push('')
    lines.push(
      `当前选中块: id=${context.selectedBlock.id} [${context.selectedBlock.type}] ${context.selectedBlock.preview}`,
    )
  ...
  lines.push(
    '- table: 传 headers 数组(如 ["列1","列2"]) 和 rows 数组(如 [["a","b"],["c","d"]])。不要创建空表格!',
  )
  ...
  lines.push(
    '所有文本类字段(text/items.text/headers/rows)均支持 Markdown 行内语法,工具会自动解析并渲染:',
  )
  ...
  lines.push(
    '- 用户提到「PPT」「幻灯片」「演示文稿」「翻页」「分页」「做几页」时,必须使用分页符(page_break)对内容进行分页。',
  )
  ...
  lines.push(
    '- 用户提到「图表」「流程图」「饼图」「柱状图」「条形图」「时序图」「序列图」「类图」「思维导图」「甘特图」「状态图」「架构图」等可视化需求时,使用 Mermaid 语法生成。',
  )
  ...
  lines.push('## 3. 数学公式')
  lines.push(
    '- 涉及数学公式时,优先使用 **块级公式**(行间公式),用 $$...$$ 包裹,独占一行,排版更美观。',
  )
  lines.push('- 行内公式($...$)仅用于句子中间的短公式或变量。')
  lines.push(
    '- 公式使用 LaTeX 语法,支持常见命令: \frac, \sqrt, \sum, \int, \alpha, \beta, \theta, \approx, \leq, \geq, \times, \div, \pm, \infty 等。',
  )
  ...
  lines.push('## 4. 图片插入')
  lines.push(
    '- 插入图片:使用 insert_block,type 设为 "image",传 src(图片路径/URL) 和 alt(描述文字)。',
  )
  lines.push(
    '- **本地图片**:用户会把图片放在文档所在目录下的 assets 文件夹里。上方已给出 assets 文件夹路径,用 list_dir 工具查看有哪些图片文件,然后用相对路径 "assets/图片名.png" 作为 src。',
  )
  lines.push(
    '- **网络图片原理**:找到公开图片的直接 URL 来插入,不是网页地址,而是图片文件本身的地址。',
  )
  lines.push(
    '  - 直接 URL 通常以 .jpg/.jpeg/.png/.gif/.webp/.svg 结尾,浏览器打开后只显示图片本身。',
  )
  lines.push('  - 常见来源:维基百科/百度百科图片、图床(CDN)、公开相册、官网素材等。')
  lines.push(
    '  - 维基百科图片 URL 格式示例: https://upload.wikimedia.org/wikipedia/commons/thumb/xxx/xxx.jpg',
  )
  lines.push('  - 错误示例: 网页地址(如 https://baike.baidu.com/item/猫) 不是图片 URL,不能直接用。')
  lines.push(
    '  - 如果无法确定可用的图片 URL,可以: ①用 web_search 搜索相关网页寻找图片资源; ②用 list_dir 查看 assets 文件夹是否有可用图片; ③直接告诉用户请提供图片 URL 或将图片放到 assets 目录。',
  )
  ...
  lines.push(
    '- 常用语言: javascript, typescript, python, java, cpp, go, rust, html, css, json, yaml, bash, sql, mermaid 等。',
  )
  ...
  lines.push(
    '- 输出前先规划好整体结构,再用 replace_document 或 batch_edit 一次性生成,避免多次零散插入。',
  )
  lines.push('')
  lines.push('# 章节操作语义（重要）')
  lines.push(
    '- "修改某个章节"指的是修改该标题及其下所有内容块(直到下一个同级或更高级标题),不只是改标题文本!',
  )
  lines.push(
    '- 用户说"修改第二章/这个章节"时,应先用 list_blocks 获取区块列表,找到该标题及其下所有块,然后用 batch_edit 批量更新。',
  )
  lines.push('- 生成表格/列表/代码块时,必须填入实际内容,不要创建空区块后就说"已完成"。')
  lines.push('')
  lines.push('# 复杂任务处理（重要）')
  lines.push(
    '- 需要一次性修改/插入/删除多个区块时,使用 batch_edit 工具,把多个操作放在 operations 数组里一次完成,而不是逐个调用工具。',
  )
  lines.push('- **replace_document 会清空用户已有内容并整体替换,属于高风险操作,必须遵守以下规则:**')
  lines.push(
    '  1. 除非用户明确要求"重写全文"/"替换全部内容"/"清空重做",否则不得主动调用 replace_document。',
  )
  lines.push(
    '  2. 当你判断需要大规模重写或重构时,必须先用文字向用户说明计划(包括为什么要替换、新结构是什么),并询问用户是否同意,得到确认后再调用 replace_document。',
  )
  lines.push('  3. 用户已有内容较多时,优先用 batch_edit 做局部增删改,而不是整体替换。')
  lines.push(
    '  4. 新建空白文档或用户明确要求从头生成内容时,可以直接用 replace_document 一次性写入。',
  )
  ...
  if (enableNativeSearch) {
    ...
    lines.push('# 联网搜索')
    lines.push(
      '你已启用原生联网搜索功能。当用户的问题涉及实时信息、最新事件、具体事实或需要查找网络资料时,你可以直接联网搜索并给出回答。',
    )
    lines.push(
      '搜索返回的是网页搜索结果(标题+摘要+网页链接),不是图片直接 URL。如需插入网络图片,请搜索相关网页后从中提取图片的直接 URL(以 .jpg/.png 等结尾)。',
    )
    ...
    lines.push(
      '使用场景: 查找实时信息、最新事件、具体事实、网络资料,或需要找图片 URL 时搜索相关网页。',
    )
    lines.push('返回格式: 每条结果含标题、摘要、网页链接。注意返回的是网页链接,不是图片直接 URL。')
    lines.push(
      '找图片时: 用 web_search 搜索相关网页,从搜索结果的网页链接中寻找图片资源,提取图片的直接 URL(以 .jpg/.png 等结尾),再用 image 区块插入。',
    )
